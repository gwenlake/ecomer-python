...
API_URL = "https://api.marinelec.gwenlake.cloud/v1"
TIMEOUT = 20
...
class Client:
    ...
    def __init__(self, api_key: Optional[str] = None):
        self._api_key  = api_key or ecomer.api_key
    ...
    def get_metrics(self, mmsi: str, start_date: str, end_date: str, aggregate: str = "1m", save_format: str = None, fields: dict = {}):
        ...
        for _start_date in _days:
            _end_date = datetime.strptime(_start_date, "%Y-%m-%d") + timedelta(days=1)
        ...
        if save_format == "csv":
            data.to_csv(f"ecomer metrics-{mmsi} from {start_date} to {end_date}.csv")
            ...
            gdf.to_file(f"ecomer metrics-{mmsi} from {start_date} to {end_date}.shp.zip")
